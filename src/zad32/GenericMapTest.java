...
class MapOps {
   public static <K, V> Map<K,V> merge(Map<K, V> map1, Map<K, V> map2, MergeStrategy<V> strat) {
        Map<K, V> result = new TreeMap<>();

        map1.forEach((k, v) -> result.merge(k, v, strat::merge));
        map2.forEach((k, v) -> result.merge(k, v, strat::merge));
