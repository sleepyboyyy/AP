...
class File implements Comparable<File>{
    ...
    public String getMonthNDay() {
        return dateOfCreation.getMonth() + "-" + dateOfCreation.getDayOfMonth();
    }

    @Override
    public int compareTo(File o) {
        return Comparator.comparing(File::getDateOfCreation)
...
class FileSystem {
    ...
    public FileSystem() {folders = new HashMap<>();}
    ...
    public Map<String, Long> sizeByMonthAndDay() {
        return folders.values().stream()
                .flatMap(Collection::stream)
                .collect(Collectors.groupingBy(File::getMonthNDay))
                .values().stream()
                .collect(Collectors.toMap(x -> x.get(0).getMonthNDay(),
                        y -> y.stream().mapToLong(File::getSize).sum()));
    }
}
